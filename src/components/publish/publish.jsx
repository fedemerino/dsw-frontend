...
import { createListing, getAmenities, getDestinations, getPaymentMethods, getImageUploadUrl } from "@/lib/api"
import { Building2, Upload, MapPin, Check, X, Loader2 } from "lucide-react"
...
export function PublishPropertyPage() {
  ...
  const [uploadingImages, setUploadingImages] = useState({}) // { imageId: true }
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [amenitiesData, paymentMethodsData] = await Promise.all([
          getAmenities(),
          getPaymentMethods()
        ])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "Error al publicar la propiedad. Por favor, intenta nuevamente.")
    } finally {
      setIsSubmitting(false)
    }
  }
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold text-foreground">Publica tu propiedad</h1>
          </div>
          <p className="text-muted-foreground">
            Completa la información de tu propiedad para empezar a recibir reservas
          </p>
        </div>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {paymentMethods.map((method) => (
                  <div key={method.id} className="flex items-start space-x-2 p-3 rounded-lg border border-border hover:bg-accent/50 transition-colors">
                    <Checkbox
                      id={method.id}
                ...
                {(formData.images.length > 0 || Object.keys(uploadingImages).length > 0) && (
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    ...
                    {formData.images.map((imageUrl, index) => (
                      <div key={imageUrl} className="relative group aspect-square rounded-lg overflow-hidden border border-border">
                        <img
                          src={imageUrl}
                          alt={`Imagen ${index + 1}`}
